...
**P1 Side Dock** (`ui/toolHud/sideDock.js` + overlay.css + 🧰 bar button): 320px glass dock, header `RoLink · 111 tools · N running`, newest-top event rows (icon, pulsing status dot, duration; expand for args/result/`codeDiff`/thumbnail), Ctrl+K palette with fuzzy search over 111 (Enter copies the `###MCP_TOOL###` template), confetti-on-success / shake-on-error, collapsed-tab + persisted state, light mode + reduced-motion.

**P2 Bottom Timeline** (`ui/toolHud/bottomTimeline.js` + 🎞 bar button): collapsed strip (`Timeline · latest · N tools`) expanding to a chronological filmstrip — block width ∝ duration, category color, running stripes; hover scrub shows the event snapshot tooltip; per-block ↻ and replay-all re-run through canonical `dispatchTool` via new `window.ROLINK.replayTool` (session guard, sequential 800ms gaps, progress toasts).

**P3 Studio hologram HUD.** MCP queue stamps `meta {eventId, category}` on every command (`commandQueue.categoryOfTool`, preserved across auto-heal re-enqueue). New `studio-plugin/src/toolVisualizer.luau` module + embedded copy in `RoLink.lua` (VISUALIZER SYNC): CoreGui top-right counters (running/done/errors), event log, per-command BillboardGui chips on target instances (floating pin fallback), SelectionBox + Beam ghost shimmer on created instances, script diff popups for `set_script_content`, tick/whoosh/buzz via built-in `rbxasset://` sounds — all pcall-guarded so HUD can never fail a tool; HUD toolbar button toggles. Rojo path (`init.plugin.luau`) loads the module optionally with the same hooks.

**Tests**: new `tests/tool-events.test.js` (8), `tests/sideDock.test.js` (10), `tests/bottomTimeline.test.js` (9), `tests/studioHud.test.js` (8, incl. 111/111 category parity Luau↔extension on both plugin copies), `mcp-server/tests/toolMeta.test.ts` (4). Full battery green: node 13 suites, vitest 11, `tsc --noEmit` clean, `bridge.py` compiles.

**P4 polish.** Icon audit: `toolRegistry.js` gains 36 per-tool glyph overrides (`iconFor()` — text-safe unicode, category fallback), so all 111 resolve a meaningful non-empty icon; SideDock passes override glyphs through. Palette groups hits by category (fixed order) with headers, counts, and per-tool preview-kind tags (`diff/table/thumbnail/…`). Category toggles: new options-page section (8 color-dot checkboxes, `chrome.storage.local rl-hidden-cats`) respected by the dock feed and timeline filmstrip (tracked-but-detached, re-attached in order on unhide; palette and Studio HUD always show everything). Bridge Dashboard: background keeps a deduped last-20 `ToolEvent` ring served in `statusObj().recent`; popup gains an Events tab (summary `N recent · running · ok · failed` + newest-first feed with status dots, category, duration). **Tests**: new `tests/p4-polish.test.js` (9: icon coverage/distinctness, hidden-store logic + storage sync, options↔registry color/order match, dock/timeline/palette/background/popup contracts). Full battery green: node 14 suites, vitest 11, `tsc --noEmit` clean.
...
**Phase 0 — Fix the root cause (impossible to overstate)**:
- `rolink-extension/core/main.js:907` now reads `c.tool, c.args`.
- The 3 no-op `blks.map(ZSParse.normalize).filter(Boolean)` call sites in main.js are replaced with `blks.filter(Boolean)` (extractAll already returns normalised objects).
- The parser's `normalize()` now also exposes `.name` and `.arguments` aliases so any future call site (legacy ZeroScript-style or new) works either way.
- New `tests/dispatch.test.js`: 5 contracts, all green. Includes the 111-tool full Zod-derived round-trip.

**Phase 1 — Bridge-side defense in depth**:
- `bridge.py`'s `call_tool` handler now synchronously rejects `name=""`, `name=null`, `name=42`, and non-object `arguments` with a structured `tool_result {ok:False, kind:"validation_error", error:"bridge: invalid tool name …"}` BEFORE any stdio spawn. The model sees the real reason and self-corrects instead of looping.
- New `ROLINK_DEBUG_DISPATCH=1` env var logs every dispatch with a SHA1 hash for correlation.
- New `tests/test_bridge_dispatch.py`: 5 contracts, all green.

**Phase 2 — Parser feature parity with ZeroScript** (the features ZeroScript has and RoLink was missing):
- `cleanLuaCall`: strips a leading `###LUA###` (with optional datamodel suffix) and trailing `###END_LUA###` from execute_luau's `code` when the model wraps the call in a JSON envelope and keeps the markers inside the string. Idempotent.
- `stripCodeChrome`: removes the `Copy ` / `json ` prefix that Kimi/GLM/Qwen code blocks leak after the opening marker.
- Whitespace-tolerant LUA marker regex accepts `### lua ###`, `###LUA---`, and the datamodel suffix `###LUA:Edit###` / `###LUA-Client###` / `###LUA-Server###` (ZeroScript-compatible). `execute_luau` calls now auto-adopt the marker-selected `datamodel_type`.
- `matchBrace`: string-aware brace matcher (skips braces inside JSON string literals). Replaces a class of silent failures where a multi_edit with `{` in its `new_text` would consume the wrong closing brace.
- `salvageCutOff`: last-resort salvage of cut-off JSON commands. Strictly conservative — refuses to close mid-string or close more than 2 closers. Wired into main.js's `parse_error` branch.
- `parseLoose` (control-character escape inside string literals) was already in `repairJSONStringValues`; the second-pass escape of newlines/tabs/CRs is preserved.
- `toolNameFromText` now also returns "execute_luau" when only a LUA marker is present (so the live-edit chip can render before the JSON is fully typed).
- 6 new fixtures (edge-16 … edge-21) + 8 new tests in `tests/parser.test.js`; 26/26 green.

**Phase 3 — Agent-loop robustness** (S8, S9, S10, S11, S12):
- S8: refused-dispatch reasons are now fed back to the model via the transactional result path, with a list of valid tool names, so the model self-corrects instead of looping.
- S9: every dispatch + result lands in `A.history` with `{role, name, args/text, ok, ts, durationMs, kind, sourceItem}`. The audit panel reads it.
- S10: the entire execute path in `dispatchTool` is now wrapped in `try/catch` — a Studio crash during a 30-line multi_edit no longer kills the agent loop, the model gets a structured `kind:"exception"` error and self-corrects.
- S11: multi-call chip placement is correct (uses `findToolBlockSpot`); each call's chip anchors at its correct location in the conversation.
- S12: `datamodel_type` and `studio_id` are read lazily at call time, not cached at the top of the dispatch. A user re-focusing Studio mid-loop gets the new DM on the next call.
- New `tests/agent_loop.test.js`: 7 contracts, all green.

**Phase 4 — Dispatch-safe audit table**:
- `docs/tool-audit.md` now has 5 columns (added **Dispatch-safe**). Generated by `npm run audit:tools`.
- New `tests/tool-samples.json` emitted at the same time: Zod-derived sample args for every one of the 111 tools.
- `tests/dispatch.test.js` round-trips the full Zod sample for every tool (not just empty args). 5/5 contracts green.
- **Audit summary: 111 tools, 97 dispatch-safe "yes", 14 "partial" (code-bearing fields the parser handles but the model could still bypass), 0 "no".**

**Files changed**: `rolink-extension/core/{parser.js, main.js, config.js}`, `rolink-extension/manifest.json`, `bridge.py`, `mcp-server/package.json`, `VERSION`, `CHANGELOG.md`, `mcp-server/scripts/generate-tool-audit-impl.ts`, `docs/tool-audit.md`, `tests/{dispatch.test.js, agent_loop.test.js, test_bridge_dispatch.py, tool-samples.json, parser.test.js}`, `rolink-extension/core/__fixtures__/tool-calls/edge-16…21.txt`.

**Tests**: 70 automated tests pass (26 parser + 10 execution + 10 provider + 5 dispatch + 7 agent_loop + 7 vitest + 5 bridge). `tsc` clean. **No automated test is the live Studio verification — that remains Phase 5 (human-in-the-loop).**
