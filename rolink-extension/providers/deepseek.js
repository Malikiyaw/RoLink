...
const ZSProvider = (() => {
  ...
  // Scope to the SITE's composer only: never match RoLink's own injected
  ...
  const getEditor = () => {
    ...
    // returned it - and barMount() then dragged the whole RoLink bar INTO the
  ...
  function enforceComposer(reason) {
    ...
    // off) without RoLink reverting their choice every frame.
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…RoLink: result truncated to fit DeepSeek's ${SEND_CAP}-character ` +
    ...
    try {
      return [...document.querySelectorAll("img")].filter(
        (im) => !im.closest(S.chatItem) &&
          // blob: = pending local preview; the alt (our "rolink_..." filename)
  ...
  //   isStarted():bool        - a RoLink session exists in this chat
