...
//    CodeMirror/Monaco), wrapped in a <div class="not-prose">. RoLink's
...
const RLProvider = (() => {
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…RoLink: result truncated to fit Arena's input limit - ` +
  ...
  // RoLink only supports DIRECT. Battle / Side by Side force a fresh A/B
  ...
  // bounces the tab), which RoLink can't drive. We nudge the mode dropdown
  ...
  function modeWarning() {
    ...
    if (um)
      return `Turn off <b>${um.label}</b> (${um.tip} in the composer) - ` +
        `RoLink only works in plain chat. ${um.label} mode uses a different ` +
  ...
  // RoLink bar out of the way: the anchored bar is transparent but still a
  ...
  // up, the anchored RoLink bar (a real, full-width element hugging the
