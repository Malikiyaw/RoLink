...
const RLProvider = (() => {
  ...
  // Scope to the SITE's composer only: skip RoLink's own injected UI (the
  ...
  const getEditor = () => {
    ...
    // barAnchor() then hugged the fixed RoLink bar over the editor. The real
  ...
  // its built-in tools over the RoLink command protocol. On K3 the only
  ...
  function nativeAgentModeOn() {
    ...
    // its own tooling instead of emitting RoLink command blocks.
    return !!(m && /swarm|agent/i.test(m.textContent || ""));
  }
  // Visible mode guard for the RoLink bar (core renderBar reads this every
  ...
  function modeWarning() {
    if (nativeAgentModeOn())
      return `Switch the model picker off <b>K3 Swarm</b> (pick <b>K3</b> or <b>Instantané</b>) - ` +
        `Kimi's own agentic mode replaces the RoLink commands with its native tools and breaks the agent loop.`;
    return "";
  }
  ...
  // Pick a model that actually speaks the RoLink command protocol, ONCE,
  ...
  return {
    ...
    // the capture is shown in the core's left-hand RoLink popup (ui.showImages).
    supportsVision: true,
    timings,
    ...
    reliableCounts: true,
    // Shown as a permanent, non-intrusive notice in the RoLink panel.
    // Kimi sometimes reaches for its OWN built-in/native tools (web search, code
    // runner, etc.) instead of emitting the RoLink command blocks that drive
    ...
    unstableWarning:
      ...
      "If it stops acting in Roblox Studio and answers in plain text or runs its own tools, remind it to use the RoLink commands - or start a new session.",
