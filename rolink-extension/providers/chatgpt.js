...
const RLProvider = (() => {
  ...
  // into data-rl-cm. syncCM() asks for a refresh; the listener is synchronous, so
  ...
  function syncCM(root) {
    ...
    try { document.dispatchEvent(new CustomEvent("rl-cm-sync")); } catch {}
  }
  function textWithout(root, excludeSel) {
    ...
    const walk = (n) => {
      ...
      if (n.classList && n.classList.contains("cm-content")) {
        const full = n.getAttribute("data-rl-cm");
        if (full !== null) {
          breakLine();
          t += full;
          breakLine();
  ...
  // Where the RoLink bar lives: INSIDE the rounded composer surface (the
  ...
  function setInputLock(on) {
    ...
    if (on) ed.setAttribute("data-rl-locked", "1");
    else ed.removeAttribute("data-rl-locked");
  }
  ...
  function streamText(item) {
    return item ? textWithout(item, ".rl-chip") : "";
  }
  ...
  function readAssistant() {
    ...
    return {
      present: true,
      reply: mds.map((m) => textWithout(m, ".rl-chip")).join("\n").trim(),
  ...
  function truncateForSend(text) {
    ...
    const marker = (what) =>
      `\n\n[…RoLink: result truncated (${what}) so it can be pasted into ` +
  ...
  const PROMPT_EXTRA = `- WHEN THE USER SENDS AN IMAGE: by default it is REFERENCE MATERIAL for the work they want done in their project - a screenshot of a bug, a mockup of the UI they want, a photo of the thing to build, a picture of what is wrong in Studio. Look at it, use it to understand what they want, and then do that work with the RoLink commands. Do NOT generate a new image from it, and do NOT treat it as an image-editing request. Only generate an image when the user EXPLICITLY asks you to create, generate, draw or edit one ("make me an image of...", "generate a texture", "edit this picture"). If what they want from the image is genuinely unclear, ask them in one short sentence rather than guessing - and never guess "they want a picture".`;
  ...
  // - like Gemini - we also mark the .markdown container with .rl-cmd-mask; the
  ...
  function findToolBlockSpot(item /*, chip */) {
    ...
    for (const mc of replies) {
      ...
      if (ranged) {
        const body = kids.filter((k) => !k.classList.contains("rl-chip"));
        if (body.length && body.every((k) => k.classList.contains("rl-tool-hide"))) {
          mc.classList.add("rl-cmd-mask-all");
        }
      }
    }
    return hidAny;
