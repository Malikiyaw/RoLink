...
// editor's true document on data-rl-cm; when present it must be used verbatim
...
const tapped = P.textWithout(cmBlock(['{"command":"multi_edit","params":{"edits":[{"old_str'], { "data-rl-cm": full }));
ok("data-rl-cm tap is preferred over rendered lines", tapped.includes(full));
...
const emptyTap = P.textWithout(cmBlock(["stale rendered text"], { "data-rl-cm": "" }));
...
// ── excludeSel ──────────────────────────────────────────────────────────────
// The core passes ".rl-chip" so its own injected chip never counts as model
// output (a chip echoing a tool name would otherwise re-trigger the call).
const withChip = P.textWithout(
  el("div", {}, [el("p", {}, ["real reply"]), el("div", { class: "rl-chip" }, ["execute_luau · 2s"])]),
  ".rl-chip"
);
