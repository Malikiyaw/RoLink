...
```
AI chat (in browser) -> RoLink Extension -> Bridge (127.0.0.1:17613) -> Roblox Studio (StudioMCP.exe) [+ optional mcp-server 3001 for extra power]
```
...
Download latest zip from **Releases** (or `git clone https://github.com/Malikiyaw/RoLink` and zip it). Extract the zip (important: **Extract All**, don't run from preview).

- `edge://extensions` or `chrome://extensions` → **Developer mode** ON → **Load unpacked** → select **`rolink-extension`** folder from the extracted zip (prebuilt, no `npm` needed). **Do NOT select the root `RoLink-main` folder — that will fail with “Manifest is missing”.**
...
### 3. Run the Bridge
- **Windows:** double-click `start.bat` from the **extracted** folder (auto-finds `py -3` → `python` → scans `LOCALAPPDATA\Programs\Python` → `winget install Python.Python.3.12` if missing, installs `websockets`, reclaims `:17613`, logs to `logs/start.log`, keeps window open).
- **macOS:** double-click `MacOS_Start.command` from the **extracted** folder (bypasses Gatekeeper: System Settings → Privacy & Security → Open Anyway once).

A small window opens = bridge running `ws://127.0.0.1:17613` (StudioMCP via `launch_studio_mcp.py`).
...
`bridge.py`, `config.json`, `launch_studio_mcp.py` (finds newest StudioMCP.exe), `start.bat`, `MacOS_Start.command`, `rolink-extension/` (zero-build), `mcp-server/` (optional), `studio-plugin/` (legacy poll fallback), `logs/`.
